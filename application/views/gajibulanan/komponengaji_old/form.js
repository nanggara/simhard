...
function komponengaji_save(){
    ...
    $.ajax({
 		...
 		success: function(response){  
 			...
 			komponengaji_alert_show(obj.message);
 			komponengaji_init(); 
 			komponengaji_search();
 			$('#progressBar-Komponengaji').hide(); 			 
 		}
...
function komponengaji_validate(){
	var komponengaji = $('#txt_komponengajimodal').val();  
	if(komponengaji.length==0){ 
		$('#alertMsgkomponengaji').attr('class','alert alert-warning alert-dismissable');
		$('#alertTextkomponengaji').html('Please Enter kelompok');
		return false;
	}else{
		$('#alertMsgkomponengaji').attr('class','alert alert-warning alert-dismissable hide');
		$('#alertTextkomponengaji').html('');
    ...
    $.ajax({
 		...
 		success: function(response){
 			if(response=='[]'){
 				...
 				$('#txt_komponengajimodal').val(obj.komponengaji);
 			}
