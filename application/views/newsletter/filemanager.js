...
function select_folder(folder){
	...
	$.ajax({	
		type: 'post',
		cache: false,
		url: "<?php echo  site_url('newsletter/file_manager');?>", 
		data: "path="+folder,
...
function new_folder(){
	...
	$.ajax({	
		type: 'post',
		cache: false,
		url: "<?php echo  site_url('newsletter/new_folder');?>", 
...
function open_folder(){ 
	$.ajax({	
		type: 'post',
		cache: false,
		url: "<?php echo  site_url('newsletter/open_folder');?>", 
		data: "request=open",
		success: function(res) 
		{  
...
function delete_folder(){
	...
	$.ajax({	
		type: 'post',
		cache: false,
		url: "<?php echo  site_url('newsletter/delete_folder');?>", 
...
function delete_file(){
	...
	$.ajax({	
		type: 'post',
		cache: false,
		url: "<?php echo  site_url('newsletter/delete_file');?>", 
...
function uploadFile() {
    ...
    xhr.upload.addEventListener("progress", uploadProgressNews, false);
    xhr.addEventListener("load", uploadCompleteNews, false);
    xhr.addEventListener("error", uploadFailedNews, false); 
    xhr.open("POST", "<?php echo site_url('newsletter/upload_file/')?>");
    xhr.send(fd); 
}
function uploadProgressNews(evt) {
	$('#file_loading').show();
}
function uploadCompleteNews(evt) {
	alertbox_file(true,'success',evt.target.responseText); 
	select_folder(gfolder);
	$('#file_loading').hide();
} 
function uploadFailedNews(evt) {   
