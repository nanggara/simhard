...
class reports extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->library("php_chiper_ex");
	...
	public function index()
	{
		...
		$kelompok = $this->input->post("slc_kelompok");
		$universitas = $this->input->post("slc_universitas");
  
		$data['ddl_datakelompok'] = $this->mdl_kelompok->kelompok_select_all('recipient');
		$data['ddl_datauniversitas'] = $this->mdl_referensi->universitas_select_all();
		$data['datauniversitas'] = $this->mdl_referensi->universitas_select_all();
		$data['datatahun'] = $this->mdl_reports->select_tahun_anggota($periode1,$periode2,$kelompok,$universitas);		
		...
		if(strlen($kelompok)>0){
			...
			if($rstkel->num_rows()){
				$data["namakelompok"] = "KELOMPOK: ".$rstkel->first_row()->kelompok;
			}
		}
		...
		if(strlen($universitas)>0){
			...
			if($rstuni->num_rows()){
				$data["namauniversitas"] = "UNIVERSITAS: ".$rstuni->first_row()->universitas;
			}
		}
	...
	{  		
		...
		$data['rowdetail'] = $this->mdl_reports->select_detail_tahun($kodeuniversitas,$tahun);
		$data['tahun'] = $tahun;
	...
	{
		$tahun = $this->php_chiper_ex->decode($this->uri->segment(3));
		$kodeuniversitas = $this->uri->segment(4);			
		...
		if(strpos($tahun, "|")){
			$arr_periode = explode("|", $tahun);
			$rowdetail = $this->mdl_reports->select_detail($kodeuniversitas,$arr_periode[0],$arr_periode[1]); 
		}else{
			$rowdetail = $this->mdl_reports->select_detail_tahun($kodeuniversitas,$tahun); 
		}
	...
	public function detailanggota()
	{
		...
		$tahun = $this->php_chiper_ex->decode($this->uri->segment(3));
		$kodeuniversitas = $this->uri->segment(4);
		$fakultas = $this->php_chiper_ex->decode($this->uri->segment(5));	
		...
		if(strpos($tahun, "|")){
			$arr_periode = explode("|", $tahun);
			$data['rowdetail'] = $this->mdl_reports->select_detailanggota_periode($kodeuniversitas,$fakultas,$arr_periode[0],$arr_periode[1]);
		}else{
			$data['rowdetail'] = $this->mdl_reports->select_detailanggota($kodeuniversitas,$fakultas,$tahun);
		}
		...
		$data['tahun'] = $tahun;
		$this->load->view('reports/detailanggota',$data);
	}
	...
	public function detailanggota_print()
	{
		...
		$fakultas = $this->php_chiper_ex->decode($fakultas);
		...
		$title = "Export Data Anggota ".$universitas.$fakultas.$tahun;
		if(strpos($tahun, "|")){
			$arr_periode = explode("|", $tahun);
			$rowdetail = $this->mdl_reports->select_detailanggota_periode($kodeuniversitas,$fakultas,$arr_periode[0],$arr_periode[1]);
			$title = "Export Data Anggota ".$universitas.$fakultas.date('m-Y',strtotime($arr_periode[0]))." s/d ".date('m-Y',strtotime($arr_periode[1]));
		}else{
		...
		foreach ($rowdetail->result() as $item){
			...
			$objset->setCellValue($col_abj[7].$row, date('d-m-Y',strtotime($item->tglmasukanggota)));
