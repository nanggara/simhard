...
class mdl_search extends CI_Model {
	...
	function form_get_all($count,$offset,$perpage,$filter){
		...
		//Jika Bukan Advanced
			...
			$where  = "where r.namalengkap like '%".$filter['txt_search']."%'";
		}
		...
		if($count){
			...
			(SELECT email FROM daftaremail WHERE regid=r.regidrec ORDER BY urut ASC OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY ) AS email,r.tipekeanggotaan,
